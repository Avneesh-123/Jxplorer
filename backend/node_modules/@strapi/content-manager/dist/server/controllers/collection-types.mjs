...
import { async, errors, setCreatorFields, parseHasPublishedVersionQueryParam, hasPublishedVersionBooleanToPublicationFilterMode, contentTypes } from '@strapi/utils';
...
import { buildDeepPopulate, getPopulateForLocalizations } from '../services/utils/populate.mjs';
 ...
 */ const extractStatusSortOrder = (sort)=>{
    if (!sort) return null;
    ...
    if (typeof sort === 'object' && sort !== null && 'status' in sort) {
        const dir = String(sort.status).toUpperCase();
 ...
 */ const removeStatusFromSort = (sort)=>{
    if (!sort) return sort;
    if (typeof sort === 'string') {
        const cleaned = sort.split(',').map((s)=>s.trim()).filter((s)=>!/^status:(ASC|DESC)$/i.test(s)).join(',');
    ...
    if (typeof sort === 'object' && sort !== null) {
        const { status: _removed, ...rest } = sort;
...
var collectionTypes = {
    async find (ctx) {
        ...
        const hasPublicationStatusFilter = query.status !== undefined || query.hasPublishedVersion !== undefined || query.publicationStatusFilter !== undefined;
        ...
        const queryWithoutStatusSort = rawStatusSortOrder ? {
            ...query,
            sort: removeStatusFromSort(query.sort)
        } : query;
        ...
        if (query.publicationFilter !== undefined) {
            ...
            if (legacy !== undefined) {
                findPageParams.publicationFilter = hasPublishedVersionBooleanToPublicationFilterMode(legacy);
            }
        }
   ...
   */ async publish (ctx) {
    ...
    async bulkFindForValidation (ctx) {
        ...
        const populate = await buildDeepPopulate(model);
    ...
    async bulkDelete (ctx) {
        ...
        const localeDocumentsIds = [
            ...new Set(documentLocales.map((document)=>document.documentId))
        ];
