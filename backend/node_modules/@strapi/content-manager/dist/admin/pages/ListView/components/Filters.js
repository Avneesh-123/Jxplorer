...
const { INJECT_LIST_VIEW_FILTERS } = hooks.HOOKS;
/**
...
const Root = ({ disabled, schema, layout, children })=>{
    ...
    const runHookWaterfall = strapiAdmin.useStrapiApp('FiltersImpl', ({ runHookWaterfall })=>runHookWaterfall);
    ...
    const displayedFilters = React__namespace.useMemo(()=>{
        ...
        const userOptions = users$1.map((user)=>({
                label: users.getDisplayName(user),
                value: user.id.toString()
            }));
        const formatted = extendedFilters.map((filter)=>{
            const next = {
                ...
                operators: filter.operators?.map((op)=>({
                        ...op,
