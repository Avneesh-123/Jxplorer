...
import { contentTypes, errors, file, async, pagination, sanitize } from '@strapi/utils';
import { toNumber, has, isNil } from 'lodash/fp';
...
var upload = (({ strapi })=>{
   ...
   */ async function findAndCountPage(query = {}) {
        const { pagination: pagination$1 = {} } = query;
        ...
        const isOffset = has('start', pagination$1) || has('limit', pagination$1);
        const isPaged = !isOffset;
        // Resolve start/limit applying defaults and the maxLimit cap.
        const { start, limit } = pagination.withDefaultPagination(pagination$1, {
            defaults: {
                offset: {
                    limit: defaultLimit
                },
        ...
        const transformed = strapi.get('query-params').transform(FILE_MODEL_UID, {
            ...query,
            pagination: undefined,
            start,
            limit
        });
        const shouldCount = resolveWithCount(pagination$1);
        ...
        const signedResults = await async.map(results, (file)=>fileService.signFileUrls(file));
        ...
        return {
            ...
            pagination: isNil(total) ? _.omit(paginationInfo, [
                'total',
                'pageCount'
            ]) : paginationInfo
        };
